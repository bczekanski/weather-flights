...
training.Kappa.val.ml <- (fmsb::Kappa.test(table(training.set$pred.glm > 0.11, training.set$CANCELLED)))$Result$estimate
...
test.Kappa.val.ml <- (fmsb::Kappa.test(table(test.set$CANCELLED, test.set$pred.glm > 0.11)))$Result$estimate

conf.mat <- table(list(test.set$pred.glm > 0.11, test.set$CANCELLED))
rownames(conf.mat) <- c("Predicted No Cancellation", "Predicted Cancellation")
...
secret.test.Kappa.val.ml <- (fmsb::Kappa.test(table(secret.test$CANCELLED, secret.test$pred.glm > 0.11)))$Result$estimate

conf.mat <- table(list(secret.test$pred.glm > 0.11, secret.test$CANCELLED))
rownames(conf.mat) <- c("Predicted No Cancellation", "Predicted Cancellation")
