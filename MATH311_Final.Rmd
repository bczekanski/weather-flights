...
set.seed(1)
# Update this 
clean.flights <- fread("~/Desktop/FlightData.csv")
...
training.set <- small.training.set
...
The airline data can be found here: https://docs.microsoft.com/en-us/machine-learning-server/r/tutorial-revoscaler-large-data-airline
...
&= \frac{1}{n}\sum_{j=1}I_j \\
&\text{where \(I_j\) is an indicator for predcited value matches true value for observation j}
\end{align*}
$$
...
Another aspect of LogitBoost is its use of decision stumps. This differs from a linear model, where a standard logistic model creates a linear relationship between log odds and inputs, LogitBoost creates a threshold for each predictor variable and uses this as a binary cutoff for prediction of our binary predicted variable. For example LogitBoost could create a threshold for distance on iteration 1 with a threshold of 500 where any flight with a distance of over 500 and any flight with a distance less than 500 is predicted as not cancelled. A logistic model assumes the same difference in log odds for going from 0 mile to 100 miles as going from 900 to 1000 miles which could have larger implications regarding fuel and plane size. These stumps allow us to capture large trends in the data with a single cutoff. A simple use of this algorithm is shown below.
...
For similar reasons we now remove PRCP, PRCP.TMIN.l32, TMAX.nyc, TMAX, WT03, MONTH, PRCP.nyc, TMIN_lag1.atl, WT01, WT01.nyc, PRCP.
...
##An Estimator
To estimate the optimal cutoff for our random forest we would ideally run a random forest on the entire training set and then look at each cutoff and find the optimal choice. However, this is computationally expensive so instead we trained a random forest on 288 random selections on one thousandth of our dataset (roughly 1000 observations) selected randomly. This is a small sample size and is likely not as representative of the true values as we would like, but we were limited to this size by run time. At each iteration we trained the model and then determined the optimal cutoff. 

We estimated the optimal cutoff as the top 3% of predicted probabilities. To determine whether or not this estimator is biased we looked at the the difference between the estimated cutoff and the true optimal cutoff for each iteration.
...
Above is a histogram of the differences between the estimated and optimal cutoffs. We do not believe that this data is normally distributed, therefore to test whether or not the true difference could be 0 we must bootstrap.

Null Hypothesis: The true mean of the difference between the optimal cutoff and our estimated cutoff is 0.
Alternative Hypothesis: The true mean of the difference between the optimal cutoff and our estimated cutoff is not 0.

``` {r}
diffs.boot <- NULL
for(i in 1:10000){
  samp <- sample(na.omit(diffs), replace = TRUE)
...
Here 0% of our boostrapped values for the difference between the estimated and optimal cutoffs are below 0 so therefore there is aproximately a 0% chance that the true difference between the estimator and the true value is 0 assuming that these data are representative which we believe they are because N is large. Therefore we conclude that this estimator is biased by rejecting the null hypothesis. 

To calculate its bias we can calculate the expected (mean) distance between the estimator and the true optimal cutoff.

```{r}
rf.bias <- mean(diffs, na.rm = TRUE)
unbiased.diffs <- NULL
for(i in 1:num.iter){
  # print(i)
  temp.df <-  cutoffs.sim %>%
  ...
  my.cutoff <- temp.df %>%
    filter(abs(Cutoff - Quantile.97) < 0.005) %>%
    .$Cutoff
  unbiased.diffs[i] <- max.cutoff - my.cutoff - rf.bias
}
mean(unbiased.diffs)
hist(unbiased.diffs)
rf.bias
...
We can therefore conclude that an unbiased estimator for the true optimal cutoff of a random forest trained on our training set to predict on our test set is the 97% quantile of the predicted probabilities - `r rf.bias`.
...
We can make an improved Multivariable Logistic Regression model by changing the cutoff to 0.11. This model is shown below.
...
training.Kappa.val.ml <- (fmsb::Kappa.test(table(training.set$pred.glm > 0.09, training.set$CANCELLED)))$Result$estimate
...
test.Kappa.val.ml <- (fmsb::Kappa.test(table(test.set$CANCELLED, test.set$pred.glm > 0.09)))$Result$estimate

conf.mat <- table(list(test.set$pred.glm > 0.09, test.set$CANCELLED))
rownames(conf.mat) <- c("Predicted No Cancellation", "Predicted Cancellation")
...
Similarly, we can make a new Random Forest model using this unbiased estimator. Here we use our model to predict on the training set and then calculate the 97% quantile of the predicted probabilities and unbias it by subtracting the bias we calculated earlier (`r rf.bias`)
...
biased.estim <- quantile(training.set$pred.ml, 0.97)
cutoff <- biased.estim - rf.bias

training.Kappa.val.ml <- (fmsb::Kappa.test(table(training.set$pred.ml > cutoff, training.set$CANCELLED)))$Result$estimate

test.set$pred.ml <- predict(alex,
                            type = "prob",
                            test.set)[2]

test.Kappa.val.ml <- (fmsb::Kappa.test(table(test.set$CANCELLED, test.set$pred.ml > cutoff)))$Result$estimate

conf.mat <- table(test.set$pred.ml > cutoff, test.set$CANCELLED)
rownames(conf.mat) <- c("Predicted No Cancellation", "Predicted Cancellation")
# FIGURE OUT CONFUSION MATRIX
kable(conf.mat, 
      caption = "Random Forest Test Set Confusion Matrix", 
...
The model that performed best on the test set was the New Multivariable Logistic Regression model with the changed cutoff. We will now run this model on the secret test set to see what true out of sample performance is like.
...
secret.test$pred.glm <- stats::predict.glm(training.model,
                                        secret.test,
                                        type = "response")

secret.test.Kappa.val.ml <- (fmsb::Kappa.test(table(secret.test$CANCELLED, secret.test$pred.glm > 0.09)))$Result$estimate

conf.mat <- table(list(secret.test$pred.glm > 0.09, secret.test$CANCELLED))
rownames(conf.mat) <- c("Predicted No Cancellation", "Predicted Cancellation")

kable(conf.mat, 
      caption = "New Multivariable Logistic Regression Secret Test Set Confusion Matrix", 
      align  = "l", 
      # row.names = TRUE,
      col.names = c("No Cancellation", "Cancellation"))

kable(data.frame(training.Kappa.val.ml, test.Kappa.val.ml, secret.test.Kappa.val.ml),
      col.names = c("Training Kappa", "Test Kappa", "Secret Test Kappa"),
      caption = "New Multivariable Logistic Regression Kappa Values")
```

This extremely low secret test Kappa value is surprising and suggests that we either started to overfit when we used the cutoff, or there is some structural change that happened in 2012 that caused flights from Logan to be delayed differently.


# Conclusion

In conclusion predicting flight cancellations is difficult. It is hard to predict these classifications because such an overwhelming majority of flights are not cancelled. This creates a strong trend in the data that is very hard to predict against and thus it is hard to correctly classify cancelled flights. Although we make use of Cohen's Kappa, our model still struggles to predict against this trend even when using more advanced algorithms. It is also interesting that logistic regression outperforms both of our machine learning algorithms when given similar amounts of data even when we introduce random forests which should capture more of the relationships between variables. We can further conclude that there must be some sort of structural change to how flights are cancelled because our model performs well on the test set but not well at all on the secret test set even though it hasn't seen either of these subsets of our data. Our model performs well on the test set (data from 2010-2011) but performs much worse on the secret test set (data from 2012) which none of our models were able to use. This shows that even though the model did not see either of these datasets when training, there must be some difference in the mechanism that determines whether a flight is cancelled from 2010/2011 to 2012 as our model does not seem to be overfit when looking at just the training and test sets. It could be interesting to look into what this structural change may have been as well as look in to cancellations at other airports and flight delay length.
