...
We are attempting to predict flight cancellations for flights leaving Boston using weather data from Boston, Atlanta, Detroit, and New York City. 
...
conf.mat <- cbind(table(test.set$CANCELLED, test.set$pred.glm > 0.5), c(0, 0))
...
can also try this with more variables and regardless of statistical significance the predictions don't change.
...
kable(fmsb::Kappa.test(conf.mat)$Result$estimate,
      col.names = "Test Kappa",
      caption = "Multivariable Logistic Regression Kappa Value")
```
...
kable(test.Kappa.val.ml,
      col.names = "Kappa Value",
      caption = "Single Variable LogitBoost Regression Kappa Value")
```
...
glm.cutoffs %>%
  ...
  facet_grid(~Set) +
  theme_bw() +
  labs(title = "Kappa vs Cutoff, with 0.97 quantile")
```
