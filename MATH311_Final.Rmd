...
While we are using all data from 2004 through 2009 as our training set, this data has over 700,000 entries and thus it is computationally expensive to train on such a large data set. We believe that we should be able to effectively train and get a good understanding of how our model is performing as long as we use at least 10,000 randomly selected entries from this training set. The training set we use to tune our model is actually only 2% of our total training data which equates to roughly 15,000 entries.
...
```{r}
# USING KAPPA BEFORE IT IS INTRODUCED???

# test.set$pred.glm <- stats::predict.glm(training.model,
#                                         test.set,
#                                         type = "response")
# fmsb::Kappa.test(table(test.set$pred.glm > 0.1, test.set$CANCELLED))$Result
```
...
kable(conf.mat, 
      caption = "Multivariable Losistic Regression Confusion Matrix", 
      align  = "l", 
      # row.names = TRUE,
...
kable(alex$results)
# alex$finalModel
# kable(table(alex$finalModel, alex$finalModel$Stump))
# kable(alex$finalModel$xNames)
kable(data.frame(alex$finalModel$Stump) %>% left_join(data.frame(feature = 1:56, Name = alex$finalModel$xNames), by = "feature"))
...
kable(alex$results)
# alex$finalModel
kable(data.frame(alex$finalModel$Stump) %>% left_join(data.frame(feature = 1:55, Name = alex$finalModel$xNames), by = "feature"))
...
alex$results
# alex$finalModel
kable(data.frame(alex$finalModel$Stump) %>% left_join(data.frame(feature = 1:57, Name = alex$finalModel$xNames), by = "feature"))
...
alex$results
# alex$finalModel
kable(data.frame(alex$finalModel$Stump) %>% left_join(data.frame(feature = 1:36, Name = alex$finalModel$xNames), by = "feature"))
